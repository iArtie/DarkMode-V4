#include "ownAlertLayer.h"

bool ownAlertLayer::init(CCPoint contentSize, float closeBtnScale, uint8_t alpha, int squareNumber) {
...
CCLabelBMFont* ownAlertLayer::createTitle(std::string text, float separatorScale, float usernameScale) {
...
CCMenuItemSpriteExtra* ownAlertLayer::createButton(const char* texture, CCPoint position, SEL_MenuHandler callback, float textureScale, float sizeMult) {
...
void ownAlertLayer::keyBackClicked() {
    onClose(nullptr);
}

void ownAlertLayer::onCloseThunk(CCObject* sender) {
    onClose(sender);
}

void ownAlertLayer::onClose(CCObject* sender) {
    setKeypadEnabled(false);
    removeFromParentAndCleanup(true);
}

void ownAlertLayer::registerWithTouchDispatcher() {
